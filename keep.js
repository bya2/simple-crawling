...
(async () => {
  ...
  const $liList = $('ul#webtoon-list-all').children('li:eq(8)'); // 태그 li의 리스트 (children: 해당되는 태그들의 배열을 반환)

  $liList.each(async (i, e) => {
    const el = $(e);

    const title = el.find('span').text(),
          url = encodeURI(el.find('a').attr('href')),
          img = encodeURI(el.find('img').attr('src'));

    const html = await getHtml(url);

    const $page = cheerio.load(html.data);

    const score = parseFloat($page('div.view-comment').text().split('\n')[5]),
